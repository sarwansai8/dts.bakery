/* ============================================================
   DTS CHOCOLATE — UI COMPONENTS LIBRARY
   Reusable component rendering functions using SVG icons
   ============================================================ */
...
const Icons = {
  ...
  starEmpty: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>`,
...
const Toast = {
  ...
  show(message, type = 'success', duration = 3500) {
    ...
    toast.innerHTML = `
      ...
      <span>${message}</span>
      <button class="toast-close" aria-label="Dismiss" onclick="this.parentElement.remove()">${Icons.x}</button>
    `;
...
const Components = {
  ...
  // Star rating
  rating(value, count = null, size = 16) {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        html += `<span class="rating-star" style="width:${size}px;height:${size}px">${Icons.star}</span>`;
      } else {
        html += `<span class="rating-star rating-star-empty" style="width:${size}px;height:${size}px">${Icons.starEmpty}</span>`;
      }
    }

    if (count !== null) {
      html += `<span class="rating-text">${value} (${count})</span>`;
    }
  ...
  productCard(product, index = 0) {
    ...
    return `
      <div class="card animate-fade-in-up stagger-${(index % 8) + 1}" data-link="/products/${product.slug}" style="cursor:pointer">
        <div style="position:relative;overflow:hidden">
          <img
            ...
            loading="lazy"
          />
          ${discount > 0 ? `<span class="badge badge-new" style="position:absolute;top:12px;left:12px">${discount}% OFF</span>` : ''}
          ${product.tags.includes('trending') ? `<span class="badge badge-warning" style="position:absolute;top:12px;right:12px">Trending</span>` : ''}
          <button
            class="btn btn-icon btn-ghost"
            style="position:absolute;bottom:12px;right:12px;background:rgba(255,255,255,0.9);backdrop-filter:blur(8px)"
            onclick="event.stopPropagation();CartActions.addItem(DBHelpers.getProduct('${product.id}'))"
        ...
        <div class="card-body">
          <p style="font-size:var(--text-xs);color:var(--color-text-muted);text-transform:uppercase;letter-spacing:0.06em;margin-bottom:4px">${product.category}</p>
          <h3 class="card-title">${product.name}</h3>
          ${Components.rating(product.avgRating, product.reviewCount)}
          <div style="display:flex;align-items:baseline;gap:var(--space-sm);margin-top:var(--space-sm)">
            <span class="card-price">${DBHelpers.formatPrice(product.price)}</span>
            ${product.compareAtPrice ? `<span class="card-price-old">${DBHelpers.formatPrice(product.compareAtPrice)}</span>` : ''}
          </div>
        </div>
      </div>
    `;
  ...
  // Quantity selector
  quantitySelector(productId, currentQty, variant = null) {
    const variantAttr = variant ? `'${variant}'` : 'null';
    return `
      <div class="qty-selector">
        <button class="qty-btn" onclick="CartActions.updateQuantity('${productId}', ${currentQty - 1}, ${variantAttr})" aria-label="Decrease quantity">${Icons.minus}</button>
        <span class="qty-value">${currentQty}</span>
        <button class="qty-btn" onclick="CartActions.updateQuantity('${productId}', ${currentQty + 1}, ${variantAttr})" aria-label="Increase quantity">${Icons.plus}</button>
      </div>
    `;
  },

  // Empty state
  emptyState(icon, title, message, ctaText = null, ctaLink = null) {
    return `
      <div class="empty-state">
        <span style="width:80px;height:80px;color:var(--color-border-strong)">${Icons[icon] || Icons.package}</span>
        <h3>${title}</h3>
        <p>${message}</p>
        ${ctaText ? `<a class="btn btn-primary" data-link="${ctaLink}">${ctaText}</a>` : ''}
      </div>
    `;
  },

  // Skeleton loader
  skeleton(width = '100%', height = '20px', radius = 'var(--radius-md)') {
  ...
  productCardSkeleton() {
    return `
      <div class="card" style="pointer-events:none">
        ...
        <div class="card-body">
          ${Components.skeleton('40%', '12px')}
          <div style="margin-top:8px">${Components.skeleton('80%', '20px')}</div>
          <div style="margin-top:8px">${Components.skeleton('60%', '14px')}</div>
          <div style="margin-top:12px">${Components.skeleton('30%', '24px')}</div>
        </div>
      </div>
    `;
  },
    ...
    return `
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ${items.map((item, i) => {
          ...
          return `<a data-link="${item.href}">${item.label}</a><span class="breadcrumb-separator">${Icons.chevronRight}</span>`;
        }).join('')}
      </nav>
    `;
  },
